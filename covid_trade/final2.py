...
eu = trade_confiremd_eu[['수출액(천불)', '누적확진자수']]
# list 형식으로 값 넣기
...
correlation_by_country.append(eu.corr().values[0][1])
correlation_by_country
#막대로 보여주기 ! 
plt.bar(x=['korea','china','taiwan','japan','usa', 'eu'], height=correlation_by_country)
...
plt.scatter(wti['월'].loc[wti['WTI'].idxmin()], wti['WTI'].min(), label='16.7', s=30,  alpha=1)
plt.scatter(wti['월'].loc[wti['WTI'].idxmax()], wti['WTI'].max(), label='72.43', color='black', s=30,  alpha=1)
plt.axhline(wti['WTI'].mean(), label='평균 값',linewidth=1, linestyle='--', color='r', alpha=0.6)
xvalues = ['20-01', '21-01', '21-08']
yvalues = [20, 30, 40, 49.2235, 60, 70]
plt.xticks(xvalues, fontsize=8.5)
plt.yticks(yvalues, fontsize=8.5)
plt.title('세계 유가 동향')
plt.legend()
